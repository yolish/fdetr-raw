...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        memory, last_attn = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed)
        tgt = torch.zeros_like(query_embed)

        # use prior to update target
        num_queries = query_embed.shape[0]
        _, s, _ = last_attn.shape
        for i in range(last_attn.shape[0]):
            attn_maps_i = simple_nms(last_attn[i, :, :].unsqueeze(0)).squeeze(0)
            # Get top-k locations
            _, indices = torch.sort(attn_maps_i.flatten(), descending=True)
            indices = indices[:num_queries]
            x = indices % s
            y = indices // s
            query_embed[:,i,:] += self.prior_pos_embed(x,y).squeeze(0)
